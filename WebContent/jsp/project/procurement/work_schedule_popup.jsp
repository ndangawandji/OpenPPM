...
function editWork(element) {	
	...
	$("#workActivityName").text(unEscape(work[3]));	
	f.<%=Activityseller.BASELINESTART %>.value 		= work[4];
	f.<%=Activityseller.STARTDATE %>.value 			= work[5];
	f.<%=Activityseller.BASELINEFINISH %>.value 	= work[6];
	f.<%=Activityseller.FINISHDATE %>.value 		= work[7];
	$("#<%=Activityseller.WORKSCHEDULEINFO %>").text(unEscape(work[8]));		
	
	$('#activityStartSchedule').text(work[10]);
	$('#activityFinishSchedule').text(work[11]);
...
<c:if test="${tl:hasPermission(user,project.status,tab)}">
	...
	<!--
		readyMethods.add(function() {
			var dates = $('#<%=Activityseller.BASELINESTART %>, #<%=Activityseller.BASELINEFINISH %>').datepicker({
