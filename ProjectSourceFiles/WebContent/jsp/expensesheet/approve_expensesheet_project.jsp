...
function viewExpenseSheet(idEmployee, idProject) {
			...
			<tbody>
				<c:forEach var="approval" items="${approvals }">
					<tr>
						...
						<td><fmt:message key="applevel.${tl:getStatusResourceExpense(approval[0], approval[2], sheetDate, app1, approveLevel, user) }"/></td>
						<td>${tl:toCurrency(tl:getCostResource(approval[0], approval[2], sheetDate, user, approveRol)) }</td>
						<td><img src="images/view.png" onclick="viewExpenseSheet(${approval[0] },${approval[2] })" title="<fmt:message key="view"/>"/></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
