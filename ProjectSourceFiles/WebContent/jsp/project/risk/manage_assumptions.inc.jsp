...
function editAssumption(element) {
	...
	var assumption = assumptionTable.fnGetData( element.parentNode.parentNode );
...
function editLogAssumption(element) {
	...
	var log = assumptionLogTable.fnGetData( element.parentNode.parentNode );
...
readyMethods.add(function() {
	...
	$('#tb_assumptions tbody tr').live('click', function (event) {
		fnSetSelectable(assumptionTable, this);
	} );
	...
	$('#tb_assumption_logs tbody tr').live('click', function (event) {
		fnSetSelectable(assumptionLogTable, this);
	} );
	...
	validatorAssumption = $("#frm_assumption").validate({
		...
		rules: {
			...
			assumption_description: {required: true, maxlength:500 }
		},
		messages: {
	...
	validatorLogAssumption = $("#frm_assumption_log").validate({
		...
		rules: {
			...
			assumption_log_change: {required: true, maxlength:500 }
		},
		messages: {
...
<fieldset class="level_1">
	<legend class="link" onclick="changeCookie('field_assumption_control')"><fmt:message key="assumptions"/></legend>
	<div class="buttons">
		<img id="field_assumption_controlBtn" onclick="changeCookie('field_assumption_control', this)" class="link" src="images/ico_mas.gif">
	</div>
	<div id="field_assumption_control" class="hide" style="padding-top:10px;">
